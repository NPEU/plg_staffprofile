...
class ImageEdit {
    ...
    protected function hasImage()
    {
        ...
        if (!$_FILES[$fieldname]['error']
            ...
            if (is_uploaded_file($_FILES[$fieldname]['tmp_name'])) {
                ...
                $this->ext         = strtolower(substr($name, strrpos($name, '.') + 1));
                $this->current_img = $this->buildImagePath($this->version);

                $uploaded_filename = $this->buildCacheName($this->version);
                move_uploaded_file($_FILES[$fieldname]['tmp_name'], $uploaded_filename);
                chmod($uploaded_filename, 0777);
                return true;
            }
        }
    ...
    {
        // This part is mainly for demo purposes. Possibly need to rethink:
        if (!$this->savepath && $this->demo) {
    ...
    protected function setMemoryForImage($filename)
    {
